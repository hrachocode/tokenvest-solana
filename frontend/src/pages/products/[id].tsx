...
import { CMS_PRODUCTS, POPULATE_ALL } from "@/constants/cms";
...
export async function getStaticPaths() {
  ...
  const { data: product = [] } = await handleRequest(`${process.env.NEXT_PUBLIC_CMS_API}${CMS_PRODUCTS}`, METHODS.GET) ?? {};
...
export async function getStaticProps({ params: { id } = {} }: GetStaticPropsContext) {
  const { data, data: { attributes } } = await handleRequest(`${process.env.NEXT_PUBLIC_CMS_API}${CMS_PRODUCTS}/${id}${POPULATE_ALL}`, METHODS.GET);
