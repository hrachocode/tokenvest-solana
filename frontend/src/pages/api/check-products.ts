...
export default async function handler({ body: { } = {} }: NextApiRequest, res: NextApiResponse) {
    ...
    const activeProducts: IProduct[] = products.filter((item: IProduct) => item.isComplete === false) || [];

    const dates: IProductDate[] = activeProducts.map((item: IProduct) => {
        const startDate = new Date(Date.parse(item.createdAt));
        const endDate = new Date(startDate.getFullYear(), startDate.getMonth(), startDate.getDate() + Number(item.days)).getTime();
        return {
            id: item.id,
